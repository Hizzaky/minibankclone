...
<div style="margin-left: 5%;">
	<div class="p-3 m-4 border border-dark position-relative d-block" style="margin-left: 10%; width:80%;">
 	...
 	<div class="form-group mb-3">
<?php 	if (is_null($data['form']) || empty($data['form']) || $data['form']=="") { ?>
	 		<!-- <form id="form" action="up.php" method="POST" enctype="multipart/form-data">
				<label class="label" for="">Rencana Anggaran Biaya (RAB)<label class="x"> *</label></label>
				<div class="row">
					<div class="col-sm-1"></div>
					<div class=" col-sm-9">
						<input class="form-control" type="text" id="rab" name="rab" placeholder="Input Tautan Kartu Keluarga" minlength="5" required> 
					</div>
					<div class=" col-sm-2">
						<button class="btn btn-primary">simpan Tautan RAB</button>
					</div>
				</div>
	 		</form> -->
...
<?php 	}else{ ?>
			<!-- <form id="form" action="del.php" method="POST" enctype="multipart/form-data">
				...
				<div class="row">
					<div class="col-sm-1"></div>
					<div class=" col-sm-9">
						<input class="form-control" type="text" id="rab" name="rab" value="<?php echo $data['rab']; ?>" minlength="5" required hidden> 
						<input class="form-control" type="text" id="" name="" value="<?php echo $data['rab']; ?>" minlength="5" required disabled> 
					</div>
					<div class=" col-sm-2">
						<button class="btn btn-danger">Hapus Tautan RAB</button>
					</div>
				</div>
	 		</form> -->
<?php	} //echo '<a style="text-decoration:none" href="../form-pengajuan">
// 	 		<button class="btn btn-outline-secondary btn-block">Form Pengajuan Pembiayaan</button>
// 	 		</a>
// 		<hr>
// ';
		...
		if ((!is_null($data['kk']) || !empty($data['kk']) || !$data['kk']=="") AND (!is_null($data['ktp']) || !empty($data['ktp']) || !$data['ktp']=="") AND (!is_null($data['form']) || !empty($data['form']) || !$data['form']=="")) { ?>
			...
			<?php elseif ($data['pengajuan_nasabah']==2): ?>
				<form action="ajuan.php" method="POST" enctype="multipart/form-data">
					<button class="btn alert alert-info form-control bt">ajukan Pembiayaan</button>
			...
			<?php elseif ($data['pengajuan_nasabah']==3): ?>
				<form action="ajuan.php" method="POST" enctype="multipart/form-data">
					<!-- <button class="btn alert alert-info form-control bt">ajukan pinjaman</button> -->
					<!-- <button class="btn alert alert-danger form-control bt " disabled >Permohonan Anda Ditolak Pihak ADMIN.</button>				 -->
				</form>	
			<?php else: ?>
					<?php if ($data['form']==1): ?>
						<form action="ajuan.php" method="POST" enctype="multipart/form-data">
